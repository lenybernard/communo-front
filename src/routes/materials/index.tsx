...
interface MaterialListProps {
    page: number
    topRef: null | MutableRefObject<HTMLInputElement>
    searchTerms: string
}
...
const MaterialList = ({ page, topRef, searchTerms }: MaterialListProps) => {
    ...
    const scrollTo = (ref: MutableRefObject<HTMLInputElement>) => {
    ...
    return (
        ...
        (error && <div>Error :(</div>) || (
            <Container maxW="8xl" ref={topRef}>
                <Text>
                    {t(
                        'material.index.itemsFound' +
                            (paginationInfo.totalCount > 1 ? '_plural' : ''),
...
export const Index = () => {
    ...
    return (
        <>
            <Helmet>
                <title>
                    {t('material.index.title')} {t('meta.title.suffix')}
                </title>
            </Helmet>
            <Box textAlign="center" fontSize="xl" ref={topRef}>
                <Grid p={4}>
                    <VStack>
                        <Heading>{t('material.index.title')}</Heading>
